...
def test_convertsnap2inference(loadmodel, outputfolder,
                               inputshape='3,112,112', embedding_size = 512):
...
def myreader_evalface(datasetfile, labeldata, batchsize):
    ...
    for key1,key2 in labeldata:
        for flip in range(2):
            ...
            ops = [[convert2rgb], [swapaxis]] if flip==0 else [[flip], [convert2rgb], [swapaxis]]
            data1 = preprocess(img1, ops)
            data2 = preprocess(img2, ops)
            batches.append(data1)
            batches.append(data2)
            if len(batches) == batchsize:
                yield np.vstack(batches).reshape((len(batches), data1.shape[0],data1.shape[1], data1.shape[2]))
                batches = []
            break
    if batches:
        np.vstack(batches).reshape((len(batches), data1.shape[0],data1.shape[1], data1.shape[2]))
...
def eval_face(facemodelpath, testnames='cfp_fp,agedb_30,lfw'):
    ...
    for testname in test_list:
        ...
        for batch in myreader_evalface(test_datasetfile, data_list, batchsize):
            [embeddings] = exe.run(test_program, fetch_list = fetch_list_test, feed={'image': batch})
            #print('embeddings', embeddings.shape, (embeddings * embeddings).sum(axis=1).shape)
            embeddings /= np.sqrt((embeddings * embeddings).sum(axis=1)).reshape((-1,1))
            all_embeddings.append(embeddings)
        
        all_embeddings = np.vstack(all_embeddings)
        _, _, accuracy, val, val_std, far = face_verification.evaluate(all_embeddings, issame_list, nrof_folds=10)
