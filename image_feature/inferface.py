...
#在网络里面减均值除方差
def preprocessimg(image, meanvalue, stdvalue):
    data_ori = fluid.layers.cast(x=image, dtype='float32')
    mean_values_numpy = np.array(meanvalue, np.float32).reshape(
    ...
    std_values_numpy = np.array(stdvalue, np.float32).reshape(
        -1, 1, 1).astype(np.float32)
...
def test_convertsnap2inference(loadmodel, outputfolder, inputshape='3,112,112'):
    embedding_size = 256
    ...
    inputdata = preprocessimg(image, meanvalue= [127.5, 127.5, 127.5], stdvalue=[128.0, 128.0, 128.0])
    model = resnet18.ResNet()
    out = model.net(input=inputdata, embedding_size=embedding_size)
    #载入预训练模型
    fluid.io.load_persistables(exe, loadmodel, main_program= fluid.default_main_program())
...
#             exe, pretrained_model, main_program=train_prog, predicate=if_exist)
    ...
    fluid.io.save_inference_model(dirname=outputfolder, feeded_var_names = [image.name], target_vars = [out], executor = exe, main_program=test_program, model_filename='model', params_filename='params')
...
def test_forward():
    ...
    result = exe.run(program, fetch_list=fetch_targets, feed={'image':inputdata})
    print result
